...
public class ProfileBuilderBolt extends BaseWindowedBolt implements Reloadable {
  ...
  /**
   * The current time based on the tuples that have been received.
   */
  private transient long currentTime;

  /**
   * The next time a flush should occur.
   */
  private transient long flushTime;
  ...
  private void setupZookeeper() {
    try {
      ...
      // this is temporary to ensure that any validation passes.  the individual bolt will
      // reinitialize stellar to dynamically pull from zookeeper
  ...
  public void declareOutputFields(OutputFieldsDeclarer declarer) {
    ...
    if(emitters.size() == 0) {
      throw new IllegalStateException("At least one `ProfileMeasurementEmitter` must be defined.");
    }
  ...
  private void doExecute(TupleWindow window) {
    ...
    for(Tuple tuple : window.get()) {
      ...
      JSONObject message = getField(MESSAGE_TUPLE_FIELD, tuple, JSONObject.class);
      ProfileConfig definition = getField(PROFILE_TUPLE_FIELD, tuple, ProfileConfig.class);
      String entity = getField(ENTITY_TUPLE_FIELD, tuple, String.class);
      Long timestamp = getField(TIMESTAMP_TUPLE_FIELD, tuple, Long.class);
  ...
  /**
   * Updates the internal 'clock' used to track the progression of time.
   *
   * @param timestamp A timestamp received within a tuple.
   */
  private void updateTime(Long timestamp) {
    ...
    if(flushTime == 0) {
      flushTime = currentTime + periodDurationMillis;
    }
  }

  /**
   * Resets the fields that track time after a flush.
   */
  ...
  public BaseWindowedBolt withTumblingWindow(BaseWindowedBolt.Duration duration) {
    ...
    // need to capture the window duration to use when setting the flush count down
