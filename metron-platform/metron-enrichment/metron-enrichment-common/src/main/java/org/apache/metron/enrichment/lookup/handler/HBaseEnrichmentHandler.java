...
public class HBaseEnrichmentHandler implements Handler<HBaseContext, EnrichmentKey, LookupKV<EnrichmentKey, EnrichmentValue>> {

  private String columnFamily;
  private HbaseConverter<EnrichmentKey, EnrichmentValue> converter = new EnrichmentConverter();

  public HBaseEnrichmentHandler(String columnFamily) {
  ...
  public Iterable<LookupKV<EnrichmentKey, EnrichmentValue>> get(
          Iterable<KeyWithContext<EnrichmentKey, HBaseContext>> keys,
    ...
    for(Result result : table.get(gets)) {
      ...
      String columnFamily = getColumnFamily(context);
      LookupKV<EnrichmentKey, EnrichmentValue> lookup = converter.fromResult(result, columnFamily);
      ret.add(lookup);
    }
    return ret;
  }
