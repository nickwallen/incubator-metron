...
public class SimpleHBaseAdapterTest {
  ...
  public void testEnrich() throws Exception {
    ...
    JSONObject actualMessage = adapter.enrich(new CacheKey("ip_dst_addr", "10.0.2.3", broSc));
    Assert.assertNotNull(actualMessage);
    Assert.assertEquals(expectedMessage, actualMessage);
  }

  @Test
  public void testEnrichMiss() throws Exception {
    SensorEnrichmentConfig broSc = JSONUtils.INSTANCE.load(sourceConfigStr, SensorEnrichmentConfig.class);
    JSONObject actualMessage = adapter.enrich(new CacheKey("test", "test", broSc));
  ...
  public void testEnrichNonStringValue() throws Exception {
    ...
    actualMessage = adapter.enrich(new CacheKey("ip_dst_addr", 10L, broSc));
    Assert.assertEquals(actualMessage,new JSONObject());
  }
  ...
  public void testMultiColumnFamilies() throws Exception {
    ...
    actualMessage = adapter.enrich(new CacheKey("ip_dst_addr", "10.0.2.4", broSc));
    Assert.assertNotNull(actualMessage);
